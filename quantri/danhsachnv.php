...
$sql_page = "SELECT * FROM  thanh_vien WHERE level=1 OR level=2 ORDER BY level DESC LIMIT $perRow,$rowsPerPage";
$query_page = mysqli_query($conn, $sql_page);
$totalRows = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM  thanh_vien WHERE level=1 OR level=2"));//tổng số bản ghi
                ...
                <table data-toggle="table" data-show-refresh="true" data-show-toggle="true" data-show-columns="true"
                    ...
                    while ($row=mysqli_fetch_array($query)){
                        ...
                        <tr style="height: 300px;">
                            ...
                            <td data-checkbox="true"><a href="quantri.php?page_layout=suanv&id_thanhvien=<?php echo $row['id_thanhvien'];?>"><?php echo $row['ho_ten'];?></a></td>
                            ...
                            <td data-sortable="true"><?php echo $row['level'];?></td>
                            <td data-sortable="true"><?php echo $row['dia_chi'];?></td>
                            <td>
                                <a href="quantri.php?page_layout=suanv&id_thanhvien=<?php $row['id_thanhvien'];?>"><span><svg class="glyph stroked brush"
                                                                                                                              style="width: 20px;height: 20px;"><use
                                                    xlink:href="#stroked-brush"/></svg></span></a>
                            </td>

                            <td>
                                <a onclick="return xoaThanhVien();" href="xoanv.php?id_thanhvien=<?php echo $row['id_thanhvien'];?>"><span><svg class="glyph stroked cancel" style="width: 20px;height: 20px;"><use
